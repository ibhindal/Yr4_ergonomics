...
widthx,widthy = I.canvas.mpl_connect('button_press_event', mouse_event)
...
#up or down and produces a line on the image appropriately
    ...
    lines[2 * q - 1,5] = np.arcsin(np.dot(va,vc) / (np.linalg.norm(va) * np.linalg.norm(vc)))
    lines[2 * q,5] = np.arcsin(np.dot(va,vd) / (np.linalg.norm(va) * np.linalg.norm(vd)))
    lines[2 * q - 1,6] = np.arcsin(np.dot(vb,ve) / (np.linalg.norm(vb) * np.linalg.norm(ve)))
    lines[2 * q,6] = np.arcsin(np.dot(vb,vf) / (np.linalg.norm(vb) * np.linalg.norm(vf)))
    lines[2 * q - 1,7] = np.arcsin(np.dot(vab,vce) / (np.linalg.norm(vab) * np.linalg.norm(vce)))
    lines[2 * q,7] = np.arcsin(np.dot(vab,vdf) / (np.linalg.norm(vab) * np.linalg.norm(vdf)))
    #n = cross(vce, vdf);
...
A2 = 3 * H1 * np.cos(omega)
plt.plot(np.array([x(6),lines(co,3) - O2]),np.array([y(- 6),lines(co - 1,4) + A2]),'color','g','linewidth',2)
>>>>>>>> 210691eb8d967309f27f4a287b0ea715871c1ab9:PhythonVersion.py
